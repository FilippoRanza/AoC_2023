...
CardMapping map_jokers(char *counter) {
    CardMapping mapping = {0};
    if (counter[1] == 0)
        return mapping;

    mapping.jokers = counter[1];
    ...
    mapping.card = max_card;
    counter[1] = 0;
    counter[mapping.card] += mapping.jokers;

    return mapping;
}

void reset_joker_mapping(char *counter, CardMapping mapping) {
    counter[1] = mapping.jokers;
    counter[mapping.card] -= mapping.jokers;
}
...
void hand_compute_points(Hand *h) {
    ...
    CardMapping mapping = map_jokers(count);
    ...
    reset_joker_mapping(count, mapping);
}
